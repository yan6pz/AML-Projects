...
def smooth_noise_using_pca(noise_data, mean, numOfComponent=2):
    #noiseless_covmat = np.dot(noiseless - mean_noiseless, (noiseless - mean_noiseless).T)/noiseless.shape[1]
    covmat = np.dot(noise_data - mean, (noise_data - mean).T) / noise_data.shape[1]
...
for index, name in enumerate(files_names[1:]):
    ...
    for numberOfComp in range(5):
        ...
        restored_set = smooth_noise_using_pca(data_set, mean_noiseless, numberOfComp)
        ...
        restored_set = smooth_noise_using_pca(data_set, mean_noisy, numberOfComp)
        mean_square_error = np.sum((restored_set - noiseless)**2)/noiseless.shape[1]
...
reconstructed_dataI = smooth_noise_using_pca(dataI_set, mean_dataI, 2).T
